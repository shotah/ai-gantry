...
const DefaultSparkPrompt = "" +
	"First: memory_recall aim/. If SELF.md has no north-star and recall is empty, recall subject aim/bootstrap. Already asked and no answer → [silent]. Never asked → ONE months-scale question (what should we be aiming at for the next few months?). [wait]. Do not invent. memory_store fact subject aim/bootstrap that you asked.\n" +
	...
	"Hours bootstrap: memory_recall pref/hours. Missing → ask sleep window, work window, extra quiet times (work is not DND). [wait]. Store preference subject pref/hours as sleep: HH:MM-HH:MM / work: HH:MM-HH:MM / quiet: …. Then continue with aims or [silent]. If hours exist, do not ask again. cron_list if needed.\n" +
