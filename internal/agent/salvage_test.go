...
func TestAgent_PrintedToolCallIsExecutedNotSpoken(t *testing.T) {
	...
	fc := &fakeCompleter{fn: func(req provider.Request) (*provider.Result, error) {
		...
		case 2:
			return &provider.Result{ToolCalls: []provider.ToolCall{
				{ID: "c2", Name: "garmin__list_activities", Arguments: `{"date":"2026-07-28"}`},
			}}, nil
		default:
	...
	tools := &unknownThenOK{
		defs: []provider.ToolDef{{Name: "garmin__list_activities"}},
		unknown: &mcp.UnknownToolError{
			Name:       "garmin__get_daily_activity",
			Hint:       "no such tool; valid garmin tools are: garmin__list_activities",
			Candidates: []string{"garmin__list_activities"},
		},
	}
	...
	if len(forced) != 3 || len(forced[1]) != 1 || forced[1][0] != "garmin__list_activities" {
...
func TestAgent_JSONReplyWithoutToolNameStaysAReply(t *testing.T) {
	...
	a, err := agent.New(agent.Options{
		Completer:    fc,
		Sessions:     newMemHistory(),
		Tools:        &fakeTools{defs: []provider.ToolDef{{Name: "garmin__get_hrv"}}},
		Model:        "m",
		MaxToolIters: 5,
	})
	if err != nil {
