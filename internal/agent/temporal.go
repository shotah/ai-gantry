...
// Prompt-only (not session history): a tagged [harness] RoleSystem after
// their words so intent is read first and NOW is still recency-weighted.
// Regenerated on every Handle. NOW and the day-part are first so
// meal/calendar planning does not slide to lunch or a future event. The
// week grid uses ISO dates so weekday-only memory ("Monday") cannot be
// reused after the week rolls.
