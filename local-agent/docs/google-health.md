...
2. Create an OAuth **Web** client; add loopback redirect for CLI auth
   (`http://127.0.0.1`).
...
That runs `gantry auth ghealth` → `google-health-mcp auth` (PKCE + loopback).
Tokens land in `data/.config/ghealth/tokens.json`. If `DEPLOY_HOST` is set,
