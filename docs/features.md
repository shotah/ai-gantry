...
- Per-turn **`[harness]`** clock after the user message (not stored in
  ...
  session: `[last contact]` — how long since the last human message. The
  header names only what is present. Gemini folds this block into the one
  system instruction (`LLM_SYSTEM_FOLD`).
